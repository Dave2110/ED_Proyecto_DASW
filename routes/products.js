...
const { cargar_parsear_productos, guardar_productos, cargar_parsear_productos_geles} = require('../data_handler'); 
...
//rutas nuevas para que sirva el catálogo  de los productos

//ruta que se trae todo de geles 
router.get('/geles', async (req, res) => {
    try {
        const products = await cargar_parsear_productos_geles(); // Utilizar la función importada
        // Verificar si hay parámetros de búsqueda
        if (req.query.search) {
            const query = req.query.search.toLowerCase();
            const filteredProducts = products.filter(product =>
        ...
        console.error("Error al obtener los productos de geles: ", error);
        res.status(500).json({ message: "Error al obtener los productos de geles." });
    }
});
